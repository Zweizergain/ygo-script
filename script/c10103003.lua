...
function c10103003.filter(c,e,tp,m,f,gc,chkf)
...
function c10103003.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local mg1=Duel.GetFusionMaterial(tp):Filter(Card.IsSetCard,c,0x337)
		local res=Duel.IsExistingMatchingCard(c10103003.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,c,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c10103003.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c,chkf)
			end
		end
...
function c10103003.fusop(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg1=Duel.GetFusionMaterial(tp):Filter(Card.IsSetCard,c,0x337)
	local sg1=Duel.GetMatchingGroup(c10103003.filter,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,c,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c10103003.filter,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,c,chkf)
	end
