...
function c10103009.spop(e,tp,eg,ep,ev,re,r,rp)
	local g,ft,op,c=Duel.GetMatchingGroup(aux.NecroValleyFilter(c10103009.spfilter),tp,LOCATION_GRAVE+LOCATION_DECK,0,nil,e,tp),Duel.GetLocationCount(tp,LOCATION_MZONE),0,e:GetHandler()
	if g:GetCount()<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10103009,1))
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c10103009.spfilter),tp,LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,e,tp):GetFirst()
	if tc then
