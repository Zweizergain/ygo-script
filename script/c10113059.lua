...
function c10113059.matfilter(c)
	return (c:IsRace(RACE_DRAGON) or bit.band(c:GetFusionType(),0x40002)==0x40002) and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial()
end
function c10113059.spfilter1(c,tp,g)
	return g:IsExists(c10113059.spfilter2,1,c,tp,c)
end
function c10113059.spfilter2(c,tp,mc)
	return (c:IsRace(RACE_DRAGON) and bit.band(mc:GetFusionType(),0x40002)==0x40002
		or bit.band(c:GetFusionType(),0x40002)==0x40002 and mc:IsRace(RACE_DRAGON))
		and Duel.GetLocationCountFromEx(tp,tp,Group.FromCards(c,mc))>0
end
function c10113059.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c10113059.matfilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,nil)
	return g:IsExists(c10113059.spfilter1,1,nil,tp,g)
end
function c10113059.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(c10113059.matfilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g1=g:FilterSelect(tp,c10113059.spfilter1,1,1,nil,tp,g)
	local mc=g1:GetFirst()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=g:FilterSelect(tp,c10113059.spfilter2,1,1,mc,tp,mc)
	g1:Merge(g2)
	c:SetMaterial(g1)
