...
function c10123010.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return (chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(1-tp) and chkc:IsFacedown()) or (chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc:IsRace(RACE_SPELLCASTER) and chkc:IsAttribute(ATTRIBUTE_WIND)) end
	if chk==0 then return Duel.GetMatchingGroupCount(c10123010.filter1,tp,0,LOCATION_SZONE,nil,e)>0 or Duel.GetMatchingGroupCount(c10123010.filter2,tp,LOCATION_MZONE,0,nil,e)>0
	end
	...
	elseif g1:GetCount()<=0 then
	  Duel.SelectOption(tp,aux.Stringid(10123010,1))
	  op=1
	else
