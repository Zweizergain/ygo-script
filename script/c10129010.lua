...
function c10129010.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	g:DeleteGroup()
	local ct1,ct2=Duel.GetLocationCount(tp,LOCATION_MZONE),Duel.GetLocationCount(1-tp,LOCATION_MZONE)
	local tg1,tg2=Group.CreateGroup(),Group.CreateGroup()
	...
	if tg1:GetCount()>ct1 and ct1>0 then
	   Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10129010,1))
	   tg1=tg1:Select(tp,ct1,ct1,nil)
	end
	if tg2:GetCount()>ct2 and ct2>0 then
	   Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10129010,1))
	   tg2=tg2:Select(tp,ct2,ct2,nil)
	end
	tg1:Merge(tg2)
	sg:Sub(tg1)
	tc=tg1:GetFirst()
