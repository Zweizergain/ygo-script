...
function c121105106.mat_filter(c)
    return false
end
...
function c121105106.filter(c,tp)
    return c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
end
function c121105106.ritual_custom_condition(c,mg,ft)
    local tp=c:GetControler()
    local g=mg:Filter(c121105106.filter,c,tp)
    return ft>-3 and g:IsExists(c121105106.ritfilter1,1,nil,c:GetLevel(),g)
end
function c121105106.ritfilter1(c,lv,mg)
    lv=lv-c:GetLevel()
    if lv<2 then return false end
    local mg2=mg:Clone()
    mg2:Remove(Card.IsRace,nil,c:GetRace())
    return mg2:IsExists(c121105106.ritfilter2,1,nil,lv,mg2)
end
function c121105106.ritfilter2(c,lv,mg)
    local clv=c:GetLevel()
    lv=lv-clv
    if lv<1 then return false end
    local mg2=mg:Clone()
    mg2:Remove(Card.IsRace,nil,c:GetRace())
    return mg2:IsExists(c121105106.ritfilter3,1,nil,lv)
end
function c121105106.ritfilter3(c,lv)
    return c:GetLevel()==lv
end
function c121105106.ritual_custom_operation(c,mg)
    local tp=c:GetControler()
    local lv=c:GetLevel()
    local g=mg:Filter(c121105106.filter,c,tp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
    local g1=g:FilterSelect(tp,c121105106.ritfilter1,1,1,nil,lv,g)
    local tc1=g1:GetFirst()
    lv=lv-tc1:GetLevel()
    g:Remove(Card.IsRace,nil,tc1:GetRace())
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
    local g2=g:FilterSelect(tp,c121105106.ritfilter2,1,1,nil,lv,g)
    local tc2=g2:GetFirst()
    lv=lv-tc2:GetLevel()
    g:Remove(Card.IsRace,nil,tc2:GetRace())
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
    local g3=g:FilterSelect(tp,c121105106.ritfilter3,1,1,nil,lv)
    g1:Merge(g2)
    g1:Merge(g3)
    c:SetMaterial(g1)
end
...
function c121105106.hspcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        local g=Duel.GetMatchingGroup(c121105106.rfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,e:GetHandler(),e)
