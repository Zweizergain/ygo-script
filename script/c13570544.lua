...
function cm.filter1(c,e,tp)
	return c:IsFaceup() and c:GetRank()==4 and c:IsRace(RACE_FIEND) and c:IsType(TYPE_XYZ)
...
function cm.filter2(c,e,tp,mc)
	...
	return c:GetRank()==6 and c:IsRace(RACE_FIEND) and c:GetDefense()==mc:GetDefense()
		and c:IsAttribute(mc:GetAttribute()) and mc:IsCanBeXyzMaterial(c)
