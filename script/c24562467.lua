...
function c24562467.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(24562467,0))
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_SUMMON_PROC)
	e1:SetCondition(c24562467.ttcon)
	e1:SetOperation(c24562467.ttop)
	e1:SetValue(1)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetCondition(c24562467.e3con)
	e3:SetTarget(c24562467.e3tg)
	e3:SetOperation(c24562467.e3op)
	c:RegisterEffect(e3)
...
function c24562467.bbfil(c,e,tp,bb)
	return c:GetEquipGroup():IsContains(bb)
end
function c24562467.e3con(e,tp,eg,ep,ev,re,r,rp)
	local tp=e:GetHandler():GetControler()
	local bb=e:GetHandler()
	local bb1=Duel.GetMatchingGroupCount(c24562467.bbfil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil,e,tp,bb)
...
function c24562467.e4tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c24562467.e4fil,tp,LOCATION_REMOVED,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_REMOVED)
end
function c24562467.e4op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 and Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 and Duel.IsExistingMatchingCard(c24562467.e4fil,tp,LOCATION_REMOVED,0,1,nil,e,tp) then return false end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c24562467.e4fil,tp,LOCATION_REMOVED,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
	local tc=g:GetFirst()
		if Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP) then
			Duel.BreakEffect()
			if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 and not Duel.GetMatchingGroupCount(c24562467.e4fil,tp,LOCATION_REMOVED,0,nil,e,tp)<=0 and not tc:IsFaceup() then return false end
	Duel.Equip(tp,c,tc,true)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_EQUIP_LIMIT)
	e1:SetReset(RESET_EVENT+0x1fe0000)
	e1:SetValue(c24562467.eqlimit)
	c:RegisterEffect(e1)
		end
	end
end
	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c24562467.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf,chkf,cv)
			end
		end
		return res
	end
...
function c24562467.e3op(e,tp,eg,ep,ev,re,r,rp)
	local cv=e:GetHandler()
	local chkf=tp
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c24562467.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf,cv)
	end
...
function c24562467.ttop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local g=Duel.SelectMatchingCard(tp,c24562467.ttfil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
