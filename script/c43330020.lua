...
function cm.tktg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetMZoneCount(tp)>=4 and Duel.IsPlayerCanSpecialSummonMonster(tp,43330026,0,0x4011,1000,2000,4,RACE_MACHINE,ATTRIBUTE_DARK) and not Duel.IsPlayerAffectedByEffect(tp,59822133) end
...
function cm.tkop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>=4 and Duel.IsPlayerCanSpecialSummonMonster(tp,43330026,0,0x4011,1000,2000,4,RACE_MACHINE,ATTRIBUTE_DARK) and not Duel.IsPlayerAffectedByEffect(tp,59822133) then
		for i=1,4 do
			local token=Duel.CreateToken(tp,43330026)
			Duel.SpecialSummonStep(token,0,tp,tp,false,false,POS_FACEUP)
			local ccode=_G["c"..43330026]
...
function cm.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,43330026,0,0x4011,1000,2000,4,RACE_MACHINE,ATTRIBUTE_DARK) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function cm.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,43330026,0,0x4011,1000,2000,4,RACE_MACHINE,ATTRIBUTE_DARK) then return end
	local token=Duel.CreateToken(tp,43330026)
...
function cm.synfilter(c)
	return c.setcard=="terraria" and c:IsNotTuner()
end
