...
function cm.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateActivation(ev) and rc:IsRelateToEffect(re) and not rc:IsImmuneToEffect(e) and rc:IsAbleToChangeControler() then
...
function cm.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,43330027,0,0x4011,2000,1000,4,RACE_MACHINE,ATTRIBUTE_DARK) and Duel.IsPlayerCanDraw(1-tp,1) end
...
function cm.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,43330027,0,0x4011,2000,1000,4,RACE_MACHINE,ATTRIBUTE_DARK) then return end
	local token=Duel.CreateToken(tp,43330027)
