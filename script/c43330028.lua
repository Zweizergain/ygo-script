--泰拉魔物 猪鲨公爵
function c43330028.initial_effect(c)
	--summon with 1 tribute
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(43330028,0))
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_SUMMON_PROC)
	e1:SetCondition(c43330028.otcon)
	e1:SetOperation(c43330028.otop)
	...
	e2:SetValue(c43330028.efilter)
	c:RegisterEffect(e2)   
	--toh
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(43330028,1))
	e6:SetCategory(CATEGORY_TOHAND+CATEGORY_DRAW)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_DAMAGE_STEP_END)
	e6:SetTarget(c43330028.thtg)
	e6:SetOperation(c43330028.thop)
	c:RegisterEffect(e6) 
end
c43330028.setcard="terraria"
function c43330028.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c43330028.thop(e,tp,eg,ep,ev,re,r,rp)
...
function c43330028.efilter(e,te)
	return te:IsActiveType(TYPE_MONSTER) and te:GetOwner()~=e:GetOwner()
end

function c43330028.otfilter(c,tp)
	return c.setcard=="terraria" and (c:IsControler(tp) or c:IsFaceup())
end
function c43330028.otcon(e,c,minc)
	if c==nil then return true end
	local tp=c:GetControler()
	local mg=Duel.GetMatchingGroup(c43330028.otfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,tp)
	return c:GetLevel()>6 and minc<=1 and Duel.CheckTribute(c,1,1,mg) and Duel.IsPlayerCanDraw(1-tp,1)
end
function c43330028.otop(e,tp,eg,ep,ev,re,r,rp,c)
	local mg=Duel.GetMatchingGroup(c43330028.otfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,tp)
