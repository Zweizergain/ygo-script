...
function c46250008.initial_effect(c)
	...
	e3:SetCondition(c46250008.descon)
...
function c46250008.tsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c46250008.spfilter,tp,LOCATION_MZONE+LOCATION_EXTRA,LOCATION_MZONE+LOCATION_EXTRA,1,nil,e:GetHandler():GetAttack()) end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,nil,1,0,0)
end
function c46250008.tspop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c46250008.spfilter,tp,LOCATION_MZONE+LOCATION_EXTRA,LOCATION_MZONE+LOCATION_EXTRA,1,2,nil,e:GetHandler():GetAttack())
	if not g then return end
	local n=math.min(Duel.Destroy(g,REASON_EFFECT,LOCATION_REMOVED),Duel.GetLocationCount(tp,LOCATION_MZONE))
	if n>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,46250001,0x1fc0,0x4011,1000,0,3,RACE_WYRM,ATTRIBUTE_DARK) and Duel.SelectYesNo(tp,aux.Stringid(46250008,0)) then
		for i=1,n do
...
function c46250008.descon(e,tp,eg,ep,ev,re,r,rp)
