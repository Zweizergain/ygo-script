--红魔✿芙兰朵露·斯卡蕾特
function c60700.initial_effect(c)
	--dis
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(60700,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetHintTiming(0,TIMING_MAIN_END)
	e1:SetRange(LOCATION_HAND)
	e1:SetCountLimit(1,60700)
	e1:SetCondition(c60700.sumcon)
	e1:SetTarget(c60700.target3)
	e1:SetOperation(c60700.operation3)
	c:RegisterEffect(e1)
	--summon/set with 1 tribute
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(60700,1))
	e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_SUMMON_PROC)
	e2:SetCondition(c60700.otcon)
	e2:SetOperation(c60700.otop)
	...
	e4:SetDescription(aux.Stringid(60700,2))
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e4:SetCode(EVENT_SUMMON_SUCCESS)
	e4:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e4:SetTarget(c60700.distarget)
	e4:SetOperation(c60700.disop)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	e6:SetDescription(aux.Stringid(60700,3))
	e6:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e6:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e6:SetCode(EVENT_DESTROYED)
	e6:SetCondition(c60700.spcon)
	e6:SetTarget(c60700.sptg)
	e6:SetOperation(c60700.spop)
	c:RegisterEffect(e6)
end
-- function c60700.GetColumnGroup(c)
...
-- end 
function c60700.otcon(e,c,minc)
	if c==nil then return true end
	return c:GetLevel()>6 and minc<=1 and Duel.CheckTribute(c,1)
end
function c60700.otop(e,tp,eg,ep,ev,re,r,rp,c)
	local sg=Duel.SelectTribute(tp,c,1,1)
	c:SetMaterial(sg)
	Duel.Release(sg,REASON_SUMMON+REASON_MATERIAL)
end
function c60700.sumcon(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	return  (ph==PHASE_MAIN1 or ph==PHASE_MAIN2) 
end
function c60700.filter2(c)
	return c:IsSetCard(0x813) and c:IsFaceup()
end
function c60700.target3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c60700.filter2,tp,LOCATION_ONFIELD,0,1,nil) 
		and e:GetHandler():IsCanBeSpecialSummoned(e,SUMMON_TYPE_SPECIAL,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c60700.operation3(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c60700.filter2,tp,LOCATION_ONFIELD,0,1,1,nil)
...
function c60700.distarget(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c60700.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
