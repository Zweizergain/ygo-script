...
function c65010038.pwfil(c)
	return c:IsSetCard(0x6da0) and c:IsType(TYPE_MONSTER) and c:IsFaceup()
end
...
function c65010038.pwop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_REMOVED,0,1,nil,0x6da0) and Duel.SelectYesNo(tp,aux.Stringid(65010038,0)) then
			Duel.BreakEffect()
			local mg=Duel.SelectMatchingCard(tp,c65010038.pwfil,tp,LOCATION_REMOVED,0,1,1,nil)
