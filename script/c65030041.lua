...
function c65030041.filter(c)
	return c:IsLevelBelow(4) and c:IsSetCard(0xdaf) and c:IsAbleToHand() and c:IsType(TYPE_MONSTER)
end
...
function c65030041.spfilter(c,at,lv)
	return c:GetAttribute()==at and c:GetLevel()==lv and c:IsAbleToHand()
end
