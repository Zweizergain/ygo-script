...
function c89380001.eqfilter2(c)
	return c:IsCode(89380001) and c:IsFaceup()
end
function c89380001.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(c89380001.eqfilter,tp,LOCATION_MZONE,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and c:IsLocation(LOCATION_GRAVE) and not Duel.IsExistingMatchingCard(c89380001.eqfilter2,tp,LOCATION_ONFIELD,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(89380001,0)) then
		Duel.BreakEffect()
